...
ENT.PrintName		= "Extension Forcefield"
ENT.Author			= "Mee / Jn"
ENT.Purpose			= ""
ENT.Instructions	= ""
ENT.Spawnable   	= true
...
function ENT:SetupDataTables()
	self:NetworkVar("Float", 0, "Strength", {KeyName = "Strength", Edit = {type = "Float", order = 1, min = -500, max = 500}})
	self:NetworkVar("Float", 1, "Radius", {KeyName = "Radius", Edit = {type = "Float", order = 2, min = 0, max = 500}})
	self:NetworkVar("Int", 1, "FalloffType", {KeyName = "FalloffType (-1 is none, 0 is linear, above 1 is less falloff)", Edit = {type = "Int", order = 2, min = -1, max = 5}})
	...
	hook.Add("gwater2_posttick", self, function()
		...
		gwater2.solver:AddForceField(self:GetPos(), self:GetRadius(), self:GetStrength(), math.Clamp(self:GetStrength(), 0, math.huge), self:GetFalloffType() == -1)
	end)
end
